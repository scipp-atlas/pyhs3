...
def _collect_all_entities(functions: Any, distributions: Any) -> list[Any]:
    """
    Collect functions, distributions, and any internal nodes they expose.

    Calls ``get_internal_nodes()`` exactly once per ``HasInternalNodes``
    distribution, so callers that need the internal nodes for more than one
    purpose (e.g. both entity-type mapping and edge building) should collect
    them once here and reuse the result, rather than looping over
    ``distributions`` again.
    ...
    Returns:
        List of functions, distributions, and their internal nodes (e.g. modifiers).
    """
    all_entities = list(functions) + list(distributions)
    for dist in distributions:
        if isinstance(dist, HasInternalNodes):
            all_entities.extend(dist.get_internal_nodes())
    return all_entities


def _map_entities(
    parameterset: Any, all_entities: list[Any]
) -> tuple[dict[str, EntityType], dict[str, TensorVar]]:
    """
    Build entity-type and constants mappings from a pre-collected entity list.

    Args:
        parameterset: Collection of parameter points
        all_entities: Functions, distributions, and internal nodes, as
            returned by :func:`_collect_all_entities`.
    ...
    # Track the kind of each "hard" entity (distribution / function / modifier),
    # i.e. one that owns a graph node, so genuine name collisions can be flagged.
    # Parameters declared in parameter_points are also tracked here, since a hard
    # entity shadowing a declared parameter is a genuine collision; implicit
    # parameters inferred from a dependency reference are not (they share the
    # same logical name as the entity they feed and are added via setdefault).
    declared_kinds: dict[str, str] = {}

    def _claim(name: str, kind: str) -> None:
        """Register a named hard entity, raising on a collision with a distinct one."""
        existing = declared_kinds.get(name)
        if existing is not None:
            msg = (
                f"Duplicate entity name '{name}': defined as both a {existing} and a "
                f"{kind}. Distributions, functions, parameters, and modifiers share a "
                f"single name namespace; each name must be unique."
            )
            raise DuplicateEntityError(msg)
        declared_kinds[name] = kind

    # Map all parameter names
    for param in parameterset:
        _claim(param.name, "parameter")
    ...
    for entity in all_entities:
        ...
        _claim(entity.name, entity_type)
        entity_types[entity.name] = entity_type

        # Also map any constants generated by this entity. Constant names are
        # derived from f"constant_{entity.name}_{field}", which is ambiguous
        # when names contain underscores (e.g. entity "a_b" field "c" and
        # entity "a" field "b_c" both yield "constant_a_b_c"). Route through
        # _claim so a genuine collision raises instead of silently
        # overwriting the earlier constant's value in constants_map.
        if isinstance(entity, Evaluable):
            for constant_name, constant_tensor in entity.constants.items():
                _claim(constant_name, "constant")
...
def build_entity_mappings(
    ...
    Returns:
        Tuple of (entity_types mapping, constants mapping)
    """
    all_entities = _collect_all_entities(functions, distributions)
    return _map_entities(parameterset, all_entities)
...
def build_dependency_graph(
    ...
    """
    # Collect functions, distributions, and internal nodes (e.g. modifiers)
    # exactly once; both the entity-type mapping below and the edge-building
    # loop further down reuse this same list instead of each re-deriving it
    # (which would call get_internal_nodes() on every distribution twice).
    all_entities = _collect_all_entities(functions, distributions)

    # Build entity mappings
    entity_types, constants_map = _map_entities(parameterset, all_entities)
    ...
    # Build modifiers map for efficient lookup. HasDependencies doesn't
    # declare `.name` itself (only concrete modifier classes do), so cast
    # back to Any after the isinstance narrowing to access it.
    modifiers_map: dict[str, Any] = {
        cast(Any, entity).name: entity
        for entity in all_entities
        if isinstance(entity, HasDependencies)
    }
